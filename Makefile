...
web: $(MAIN)
	...
	@python3 -c "import pathlib,shutil; p=pathlib.Path('$(WEB_OUT)'); s=p.read_text(encoding='utf-8'); css=pathlib.Path('../模板/web.css').read_text(encoding='utf-8'); p.write_text(s.replace('/*UH_WEB_CSS*/', css), encoding='utf-8'); d=p.parent/'webfonts'; d.mkdir(exist_ok=True); shutil.copy('../模板/webfonts/duan-kaixiao-subset.woff2', d/'duan-kaixiao-subset.woff2')"
